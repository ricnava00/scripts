...
#FIX: same note that plays in succession gets merged
$step=12; //rotate every nth step
$multiply=1; //duplicate note events, multiplies tempo (rotates more times in same period)
$direction=0; //Select priority to notes that start at same time 0: higher first, 1: lower first
$speed=1; //Scale speed

$midi = new Midi();
$midi->importMid("mid.mid");
/*
var_export($midi); exit;
*/
$out=[];
$notes=[];
$end=0;
/*
foreach(range(0,$midi->getTrackCount()-1) as $trackN)
{
	$track=array_map(fn($f)=>explode(" ",$f),$midi->getTrack($trackN));
	$track=array_values(array_filter($track,fn($f)=>!($f[1]=="Par"&&$f[4]=="v=0")));
	$track=array_map(fn($f)=>implode(" ",$f),$track);
	$midi->setTrack($trackN,$track);
}
$midi->soloTrack(2);
...
foreach(range(0,$midi->getTrackCount()-1) as $trackN)
{
	$more=array_map(fn($f)=>explode(" ",$f),$midi->getTrack($trackN));
	$meta=current(array_filter($more,fn($f)=>$f[1]=="Meta"&&$f[2]=="TrkEnd"));
	$tempo=array_map(fn($f)=>implode(" ",$f),array_filter($more,fn($f)=>in_array($f[1],["TimeSig"])));
	if(!empty($tempo))
	{
		$out=$tempo;
	}
	$end=max($end,$meta[0]);
	$tnotes=array_filter($more,fn($f)=>in_array($f[1],["On","Off","Par"]));
	foreach($tnotes as $note)
	{
		if($note[1]!="Par"||$note[4]=="v=0")
		{
			if($note[1]!="Par")
			{
				$n=explode("=",$note[3])[1];
			...
			if(isset($n))
			{
				$notes[$note[0]][]=['on'=>!($note[1]=="Off"||$note[4]=="v=0"),'n'=>$n,'v'=>explode("=",$note[4])[1]];
			}
		}
	}
}
ksort($notes);
#var_export($notes); exit;
$playing=[];
$out=array_merge($out,['0 Par ch=1 c=127 v=0','0 PrCh ch=1 p=0']);
for($l=0;$l<array_keys($notes)[count($notes)-1]*$multiply;$l++)
{
	$n=floor($l/$multiply);
	if(isset($notes[$n]))
	{
		usort($notes[$n],fn($a,$b)=>$direction&&$a['n']<=>$b['n']);
		foreach($notes[$n] as $note)
		{
			$playing=array_filter($playing,fn($n)=>$n['n']!=$note['n']);
			if($note['on'])
			{
				unset($note['on']);
				array_unshift($playing,$note);
			}
		}
	}
	if(!empty($playing))
	{
		$note=array_shift($playing);
		if(!isset($last)||$note['n']!=$last)
		{
			if(isset($last))
			{
				$out[]=$l." Off ch=1 n=".$last." v=0";
			}
			$out[]=$l." On ch=1 n=".$note['n']." v=".$note['v'];
		}
		$last=$note['n'];
		if(($l+1)%$step==0||(isset($last)&&$note['n']!=$last))
		{
			$playing[]=$note;
		}
		else
...
$out[]=$end*$multiply.' Meta TrkEnd';
$out=array_values($out);
#var_export($out);
$new=new Midi();
$new->open($midi->getTimebase());
$new->newTrack();
foreach($out as $msg)
{
	$new->addMsg(0,$msg,0);
}
$new->setTempo($midi->getTempo()/$multiply/$speed);
$new->saveMidFile("out.mid");
